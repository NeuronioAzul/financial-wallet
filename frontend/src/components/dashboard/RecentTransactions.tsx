...
export const RecentTransactions = forwardRef<
...
>((_, ref) => {
  ...
  const getTransactionColor = (transaction: Transaction) => {
    ...
    if (typeStr === "transfer") {
      const isReceived = transaction.receiver_user_id === userId;
      return isReceived
        ? "text-mint-green bg-mint-green/10"
    ...
    return (
      colors[typeStr as keyof typeof colors] ||
      "text-charcoal-gray bg-silver-gray/20"
    );
  };
  ...
  const getStatusBadge = (status: string | number) => {
    ...
    const badges = {
      ...
      pending: {
        text: "Pendente",
        color: "bg-golden-sand/20 text-golden-sand-dark",
      },
      failed: { text: "Falhou", color: "bg-burgundy-red/10 text-burgundy-red" },
      reversed: {
        text: "Estornada",
        color: "bg-silver-gray text-charcoal-gray",
      },
    };
